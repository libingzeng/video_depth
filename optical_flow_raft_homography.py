...
from utils.helpers import set_requires_grad
...
def infer(args, Flownet, device, img1_name, img2_name, size=None):
    ...
    if args.homography != 1:
        ...
        flow = Flownet(torch.squeeze(img1, 2), torch.squeeze(img2, 2), 120, test_mode=True)[1][0].permute(1, 2, 0).data.cpu().numpy()
    else:
        sz = img1.size()
        img1_view = img1.view(sz[0], sz[1], 1, sz[2], sz[3])
        img2_reg_view = img2_reg.view(sz[0], sz[1], 1, sz[2], sz[3])
        flow = Flownet(torch.squeeze(img1, 2), torch.squeeze(img2_reg, 2), 120, test_mode=True)[1][0].permute(1, 2, 0)
...
def process(args):
    ...
    Flownet.to(device)
    Flownet.eval()
    set_requires_grad(Flownet, False)
    ...
    for im1, im2, out, reg_out, hba_out in zip(args.im1, args.im2, args.out, args.reg_out, args.hba_out):
        ...
        flow, img2_reg, H_BA = infer(args, Flownet, device, im1, im2)
        ...
        if args.visualize:
            ...
            cv2.imwrite(reg_out.split('.')[0]+'.png', img2_reg)
