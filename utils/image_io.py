...
# lbz modified max_size and min_size as 384, 256 respectively
    ...
    with Image.open(file_name) as img:
        ...
        if max_size is not None:
            if min_size is not None:
                img = cv2.resize(
                    img, (min_size, max_size), interpolation=cv2.INTER_AREA) # lbz modified (max_size, min_size) to (min_size, max_size)
            else:
