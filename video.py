...
def sample_pairs_2(out_dir, pos_dist_min, frame_range, flow_ops):
    ##: lbz modified
    name_mode_map = frame_sampling.SamplePairsMode.name_mode_map()
    opts = [
        frame_sampling.SamplePairsOptions(mode=name_mode_map[op]) for op in flow_ops
    ]
    pairs = frame_sampling.SamplePairs.sample_2(
        opts, frame_range=frame_range, two_way=True, out_dir=out_dir, pos_dist_min=pos_dist_min,
    )
    print(f"Sampled {len(pairs)} frame pairs.")
    return pairs
